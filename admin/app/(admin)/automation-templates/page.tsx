...
export default function AutomationTemplatesPage() {
    ...
    return (
        ...
        <div className="pb-12 fade-in" >
            ...
            {/* Stat Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10" style={{ marginTop: '20px' }}>
            ...
            {/* Filters Row */}
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6" style={{ marginTop: '20px' }}>
            ...
            {/* Category Chips */}
            <div className="flex flex-wrap items-center gap-2 mb-8" style={{ marginTop: '20px' }}>
                {categories.map(cat => (
                    <button
                        key={cat}
                        onClick={() => setCategoryFilter(cat)}
                        className={`px-6 py-2 rounded-full border text-[13px] font-bold transition-all ${categoryFilter === cat
            ...
            {/* Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" style={{ marginTop: '20px' }}>
                {loading && (
                ...
                {!loading && filteredTemplates.map((t, idx) => (
                    <div key={idx} className="bg-white border border-slate-100 rounded-[32px] p-8 hover:shadow-2xl hover:shadow-slate-200/60 transition-all duration-500 group relative flex flex-col">
                        ...
                        <div className="flex justify-between items-start" style={{ marginBottom: '20px' }}>
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${t.type === 'Marketing' ? 'bg-orange-50 text-orange-400' :
                                ...
                                {t.type === 'Marketing' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" /></svg>}
                                {t.type === 'Sales' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2" ry="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></svg>}
                                {t.type === 'Finance' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="M7 15h0M2 9.5h20" /></svg>}
                                {t.type === 'Content' && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>}
                                {!['Marketing', 'Sales', 'Finance', 'Content'].includes(t.type) && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /></svg>}
                            </div>
                        ...
                        <div className="flex-1 cursor-pointer" onClick={() => { setEditingTemplate(t); setShowModal(true); }}>
                            <h3 className="text-[20px] font-bold text-slate-800 tracking-tight leading-tight group-hover:text-[#4a8df8] transition-colors" style={{ marginBottom: '12px' }}>{t.title}</h3>
                            <p className="text-[14px] font-medium text-slate-500 leading-relaxed line-clamp-3" style={{ marginBottom: '20px' }}>
                        ...
                        <div className="flex items-center justify-between" style={{ paddingTop: '16px', marginTop: '4px', borderTop: '1px solid #f1f5f9' }}>
