...
import { ImplementationModal } from '../../components/Modals';
...
export default function ImplementationPage() {
    ...
    useEffect(() => {
        adminFetch<any[]>('/cms/implementation-templates')
    ...
    return (
        <div className="pb-6 fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-[28px] font-bold text-slate-800 tracking-tight">Implementation Services CMS</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">Manage deployment, training, and maintenance packages</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                <div className="card p-5">
                    <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Service Packages</h3>
                    <div className="text-3xl font-black text-slate-800 tracking-tight mb-2">{loading ? '-' : templates.length}</div>
                </div>
                <div className="card p-5">
                    <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Active Clients</h3>
                    <div className="text-3xl font-black text-emerald-500 tracking-tight mb-2">-</div>
                </div>
                <div className="card p-5">
                    <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Most Popular</h3>
            ...
            <div className="flex items-center justify-between mb-4">
                <p className="text-[13px] text-slate-500 font-medium">Service packages shown to clients during the deployment phase</p>
                <button 
                    ...
                    <div key={idx} className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                        <div className="p-5 flex-1">
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-[11px] font-bold text-purple-600 flex items-center gap-1.5 bg-purple-50 px-2 py-0.5 rounded-full capitalize">
                                    <span className="w-1.5 h-1.5 bg-purple-500 rounded-full"></span> Implementation
                                </span>
                                <span className="text-[12px] font-bold text-slate-500">#{idx + 1}</span>
                            </div>
                            <h3 className="text-[16px] font-bold text-slate-800 mb-2 leading-tight">{t.title || 'Dynamic Package'}</h3>
                            <p className="text-[13px] font-medium text-slate-500 mb-3 leading-relaxed">
                                {t.use_case || 'Dynamic description generated from backend records.'}
                            </p>
                            
                            <div className="flex justify-between items-end mt-6 border-t border-slate-100 pt-4">
                                <div>
                                    <div className="text-[10px] font-bold text-slate-400 tracking-wide uppercase mb-1">Duration</div>
                                    <div className="text-[14px] font-black text-slate-800">Custom</div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[10px] font-bold text-slate-400 tracking-wide uppercase mb-1">Base Fee</div>
                                    <div className="text-[14px] font-black text-slate-800">Calculated</div>
                                </div>
                            </div>
                        </div>
            ...
            {showModal && <ImplementationModal onClose={() => setShowModal(false)} />}        {showCreateModal && <CreateBookingModal onClose={() => setShowCreateModal(false)} />}
