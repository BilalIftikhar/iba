...
export function CreateBookingModal({ onClose, onCreated }: Props) {
    ...
    useEffect(() => {
        adminFetch<any>('/customers?limit=100')
            .then(res => {
                // Determine if backend wrapped it in success:true/data map
    ...
    const handleCreate = async () => {
        ...
        setSaving(true);
        try {
            // Find matched customer to send email if needed
            const selectedCust = customers.find(c => c.id === form.customer_id);
            await adminFetch('/bookings', {
                method: 'POST',
                body: JSON.stringify({
                    client_email: selectedCust?.email || '', // legacy API mapped
                    ...form,
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[760px] flex flex-col max-h-[90vh] overflow-hidden panel-fade-in">
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-7">
                    ...
                    <div className="flex items-center gap-4 py-2">
                        <div className="px-3 py-1 bg-amber-50 rounded-md shrink-0">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full border border-slate-100 bg-slate-50 rounded-lg py-3.5 px-4 text-[14px] font-medium text-slate-800 outline-none focus:border-[#4a8df8] focus:bg-white transition-colors min-h-[90px] resize-y shadow-inner placeholder:text-slate-400"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            value={form.notify_customer}
                            onChange={(e) => setForm(f => ({ ...f, notify_customer: e.target.value }))}
                ...
                {/* Footer */}
                <div className="px-8 py-6 border-t border-slate-100 bg-white flex items-center gap-3 shrink-0">
                    <button
                        onClick={handleCreate}
                        disabled={saving || !form.customer_id || !form.title}
