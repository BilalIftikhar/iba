...
export default function SubscriptionClient() {
    ...
    useEffect(() => {
        fetchSubscription().then((res: any) => {
            if (res && res.data) setSub(res.data);
        }).catch(console.error);
    }, []);

    const planName = sub?.plan ? sub.plan.charAt(0).toUpperCase() + sub.plan.slice(1) : 'Loading...';
    const planPrice = sub?.plan === 'pro' ? '$49' : sub?.plan === 'enterprise' ? '$299' : '$0';
        ...
        <div className="min-h-screen bg-[#F8FAFC]">
            ...
            <div className="max-w-[1240px] mx-auto md:p-8 p-4 pt-0 lg:pt-0">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-1 md:mb-3">
                                    <h2 className="text-[18px] md:text-[28px] font-bold text-[#1E293B] leading-tight flex items-center gap-2">
                                        {planName} Plan
                                        {sub && (
                                            <span className="hidden md:flex bg-[#E6F8F9] text-[#00C2FF] text-[10px] font-bold px-2 py-1 rounded tracking-wider uppercase items-center h-fit ml-2">
                                                {sub.status}
                                            </span>
                                        )}
                                    </h2>
                                    ...
                                    <div className="md:hidden flex flex-col items-end">
                                        <div className="text-[16px] font-bold text-[#00C2FF]">{planPrice}.00<span className="text-[12px] text-slate-400 font-medium">/mo</span></div>
                                    </div>
                                </div>
                                <div className="text-[13px] md:text-[15px] text-slate-500 mb-4 md:mb-6">
                                    ...
                                    <strong className="text-[#1E293B] font-medium">{sub?.next_billing_date ? new Date(sub.next_billing_date).toLocaleDateString() : 'N/A'}</strong>
                                </div>
                                <div className="hidden md:flex items-end gap-1 mb-8">
                                    <span className="text-[40px] font-black text-[#1E293B] leading-none tracking-tighter">{planPrice}</span>
                    ...
                    <div className="w-full lg:w-[480px]">
                        ...
                        <div className="grid grid-cols-2 gap-3 md:gap-4 h-full">
                            <UsageCard
                                ...
                                value={sub?.automations_used || 0}
                                total={sub?.automations_limit || 10}
                                percentage={Math.round(((sub?.automations_used || 0) / (sub?.automations_limit || 10)) * 100)}
                            />
                            <UsageCard
                                ...
                                label="API Calls"
                                iconColor="text-[#00C2FF]"
                                value={((sub?.usage?.api_calls || 0) / 1000).toFixed(1) + 'k'}
                                total={isPro ? "100k" : "25k"}
                                percentage={Math.round(((sub?.usage?.api_calls || 0) / (isPro ? 100000 : 25000)) * 100)}
                            />
                            <UsageCard
                                ...
                                label="Storage"
                                iconColor="text-[#00C2FF]"
                                value={sub?.usage?.storage || 0}
                                total={isPro ? "100 GB" : "10 GB"}
                                percentage={Math.round(((sub?.usage?.storage || 0) / (isPro ? 100 : 10)) * 100)}
                            />
                            <UsageCard
                                ...
                                label="Team Seats"
                                iconColor="text-[#00C2FF]"
                                value={sub?.usage?.team_seats_used || 1}
                                total={isPro ? "5" : "1"}
                                percentage={Math.round(((sub?.usage?.team_seats_used || 1) / (isPro ? 5 : 1)) * 100)}
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-16 px-1 md:px-0">
                    ...
                    {/* Pro */}
                    <div className={`bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 flex flex-col justify-between border-2 ${sub?.plan === 'pro' ? 'border-[#00C2FF] shadow-[0_8px_24px_rgba(0,194,255,0.12)] relative' : 'border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]'}`}>
                        {sub?.plan === 'pro' && (
                        ...
                        {sub?.plan === 'pro' ? (
                            ...
                            <button className="w-full bg-[#00C2FF] text-white font-semibold py-2.5 rounded-xl text-[14px] hover:bg-[#00A3D9] transition-colors">
                                Upgrade to Pro
                            </button>
                        )}
                    </div>
